...
@SerializableAs("miningCondition")
public class MiningCondition extends Condition {

    private int requiredLevel;

    public MiningCondition(int requiredLevel) {
        super.type = Type.MCMMO_MINING_LEVEL_CONDITION;
        this.requiredLevel = requiredLevel;
    }

    @Override
    public boolean conditionAchieved(Player player) {
        return UtilMcMMO.getSkillLevel(player, UtilMcMMO.SkillType.BERGBAU) >= requiredLevel;
    }

    @Override
    public List<String> requirementsAsLore(Player player) {
        List<String> lore = Lists.newArrayList();
        lore.add("§7Bergbau Level: §e" + UtilMcMMO.getSkillLevel(player, UtilMcMMO.SkillType.BERGBAU) + "§8/§e" + requiredLevel);
