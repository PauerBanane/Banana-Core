...
@SerializableAs("fishingCondition")
public class FishingCondition extends Condition {

    private int requiredLevel;

    public FishingCondition(int requiredLevel) {
        super.type = Type.MCMMO_FISHING_LEVEL_CONDITION;
        this.requiredLevel = requiredLevel;
    }

    @Override
    public boolean conditionAchieved(Player player) {
        return UtilMcMMO.getSkillLevel(player, UtilMcMMO.SkillType.ANGELN) >= requiredLevel;
    }

    @Override
    public List<String> requirementsAsLore(Player player) {
        List<String> lore = Lists.newArrayList();
        lore.add("§7Fischen Level: §e" + UtilMcMMO.getSkillLevel(player, UtilMcMMO.SkillType.ANGELN) + "§8/§e" + requiredLevel);
